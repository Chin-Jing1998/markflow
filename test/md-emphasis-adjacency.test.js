 ...
 *     T 内部相邻两类形态的矩阵。
 *   - 邻居产物为空：格式节点之间或之前夹着空文本、无内容的格式节点时，render() 先行剔除这些节点，格式节点按真实
 *     邻居判定；同类型嵌套规则在该类中暴露的倒退样例。
 *   - 已知行为变化：纯字母内容的相邻强调、同类型嵌套的内层均改写为标签，重新解析后格式不变；段首空文本后接的加粗
 *     曾因前一字为空串改写为标签，现空文本先行剔除，产物回到定界符。
...
test('邻居产物为空：格式节点之间或之前夹着空文本、无内容的格式节点时，这些节点先行剔除，格式节点按真实邻居判定，重新解析后格式不变', async () => {
    // Arrange：f4a1567 之前依次为「**A***B*」「甲**《乙》**」「**加粗***。*」「**A****B**」，后三例重新解析残留字面
    // 星号；f4a1567 与 d5d6823 让紧邻空节点的一方一律回退标签。现空节点在 stringify 之前剔除，各例产物与去掉空节点后的
    // IR 相同：第 1、3、4 例的两个格式节点直接相邻，只有前一方因同字符粘连回退标签；第 2 例同
    // [text(甲), strong(《乙》)]，加粗以书名号开头而前一字为汉字，仍回退标签
    const cases = [
        { children: [strong('A'), text(''), emphasis('B')], md: '<strong>A</strong>*B*\n' },
        { children: [text('甲'), text(''), strong('《乙》')], md: '甲<strong>《乙》</strong>\n' },
        { children: [strong('加粗'), text(''), emphasis('。')], md: '<strong>加粗</strong>*。*\n' },
        { children: [strong('A'), del(), strong('B')], md: '<strong>A</strong>**B**\n' },
    ];
...
test('已知行为变化：段首空文本后接的加粗曾因前一字为空串改写为标签，现空文本先行剔除、按真实邻居判定，产物回到「**加粗**」，重新解析后格式不变', async () => {
    // Arrange：f4a1567 与 d5d6823 的产物为「<strong>加粗</strong>」；现空文本在 stringify 之前剔除，加粗成为段首节点，
    // 产物与 f4a1567 之前相同
