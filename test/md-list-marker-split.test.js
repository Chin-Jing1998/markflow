 ...
 * 切在换行处的两行记号并入同一 value，「.」「-」「+」模式的 after 吞掉行尾换行，使修复前往返正确的形态出错（见两行
 * 记号的回归护栏），也会把任意相邻 text 中的转义位置集中到一次 safe() 调用，耗时成为平方级。缺陷类用例的切分点位于
 ...
 *     或结尾的文本（只作段落直属文本，每段至多一个，以换行开头的只取「)」记号）与普通文本；段首七成、硬换行之后与
 ...
 *     代码；
 *   - 段尾的硬换行：产物「甲\」加换行重新解析为字面反斜杠，硬换行丢失；
 *   - 同一个 text 节点内，只有「数字 + .」「-」或「+」的一行紧接换行、下一行行首又是同一种记号加空白：这三种模式的
 *     after 可含换行，把行尾换行吃进前一次匹配，safe() 逐模式做不重叠的全局匹配，下一行缺少 atBreak 所需的换行，记号
 *     不转义，如 [text('0.' + 换行 + '1. 项')] 的产物为「0\.」加换行再加「1. 项」，第二行成为列表（「*」另有行内规则
 *     恒转义，不受影响）。数字与记号分属两个节点、而记号与下一行同在后一节点时（如 [text('1'), text('.' + 换行 +
 *     '1. 项')]），数字边界合并后即成此形态：修复前第一行成为列表，修复后第二行成为列表。切分点在换行处的两行不在
 *     数字边界上、不会合并，修复前后都往返正确，由两行记号的回归护栏覆盖；随机文本池中以换行开头的文本只取「)」记号
 *     （「)」模式没有 after 条件，不吃换行），使随机往返不涉及这类形态；
...
// 只取「)」记号，使随机往返不涉及两行记号形态：只有「数字 + .」的一行与下一行同处一个 value 时，「.」模式的 after
// 吞掉行尾换行，下一行的「数字 + . + 空白」不再转义（范围外，见文件头）。以换行开头的文本不在数字边界上，修复不会把
// 它与前文合并；切在换行处的两行记号另由两行记号的回归护栏覆盖
