 ...
 *     换行处的两行由两行记号的回归护栏覆盖；
 *   - root 直接挂行内节点：本文件写就时属范围外——pruneEmptyInline 不处理 root 的子节点（与 d332ef0 剔除空节点的范围
 *     一致），其下相邻 text 不合并，如 root 之下直接挂 [text('1'), text('. 项')] 仍输出「1. 项」、重新解析为有序列表——
 *     现已由 render() 在 root 的子节点含上游 phrasing 类型时把全部子节点先包进段落修复，覆盖见
 *     test/md-root-phrasing.test.js；常规 IR 的行内节点总在段落、标题或单元格之内，本文件仍只渲染段落与标题；
