 ...
 * 修法：后一兄弟的产物为空时，定界符式格式节点回退 HTML 标签；peek 不改，以免以反斜杠结尾的文本失去转义。前侧
 * （前一兄弟产物为空）已由邻居产物为空的规则（f4a1567）回退标签，本文件的镜像形态作其护栏。
 * 覆盖：
 ...
 *   - 已知行为变化：修复前往返即正确、修复后因后一兄弟产物为空而由定界符改写为标签的形态。
 *   - 回归护栏：以反斜杠结尾的文本后接空节点、再后为 ASCII 标点时反斜杠仍须转义（peek 若改报空串即会失败）；不含
 *     空节点的形态产物逐字不变。
 ...
 * 「1.」或空白开头的文本、段尾空节点之前以空白结尾的文本，本修法不处理，本文件也不覆盖：矩阵中带空格的邻居只在朝向
 * E 的一侧含空格；随机文本池不含行首敏感的开头与首尾空白，也不生成段落或格式节点直属的空文本兄弟（空文本只作为空
 * 格式节点的唯一子节点出现）。
 */
...
test('已知行为变化：后一兄弟产物为空时，修复前往返即正确的定界符（如「**甲**乙」、段尾的「**《甲》**」）现改写为标签，重新解析后格式与文本均不变', async () => {
    // Arrange：修复前依次为「**甲**乙」「**《甲》**」「~~甲。~~。」，重新解析同样正确；修复后只要后一兄弟产物为空即回退
    // 标签，不论其 peek 报出何种字符、闭定界符在真实邻居旁是否仍能闭合
    const cases = [
        { children: [strong('甲'), del(), text('乙')], md: '<strong>甲</strong>乙\n' },
        { children: [strong('《甲》'), html('')], md: '<strong>《甲》</strong>\n' },
        { children: [del('甲。'), underline(), text('。')], md: '<del>甲。</del>。\n' },
    ];
...
test('回归护栏：以反斜杠结尾的文本后接空节点、再后为 ASCII 标点（闭定界符或字面星号）时，反斜杠仍须转义，重新解析后格式与文本均不变', async () => {
    // 修复前后产物均依次为「**甲\\**」与「甲\\\*乙*丙*」：空节点的 peek 报出「~」，文本末尾的反斜杠按后接 ASCII 标点转义。
