...
| 五书分文件 | 工作目录用可读名 `claims.xml` / `description.xml` / `drawings.xml` / `abstract.xml` / `abstract-figure.xml` | 八条骨架常量 + 手册 3.3.1 | A | 定稿 |
...
| **提交包布局** | 案卷包根 → 每类文书一个表格代码目录 → 目录内平铺 XML 与图片 → 根下放 `List.xml`（根元素 `data-bus`） | 同上规范第一节示例 + `WordToolKit.dll` 中与 `List.xml` 元素名逐个对应的常量串 | A | **本轮定稿**（原「待 R2 核对」） |
...
| 提交包内的图片文件名 | **四位序号** `0001.jpg`、`0002.jpg`……，按在该 XML 中的出现顺序编号；与工作目录的中间文件名（`drawing-N.jpg` 等）不同，落包时需重命名 | 规范第一节结构示例 | A | **本轮新增** |
...
| 公式图片文件名 | `omath-<段序>-<段内序>.jpg` | DLL 常量 `\omath-` 与完整实例 `\omath-1-1.jpg` | A | 定稿 |
| 附图图片文件名（工作目录） | `drawing-<序>.jpg` | DLL 常量 `\drawing-` | B | 定稿（序号起点待核；落包时改为 `000N.jpg`） |
| 表格图片文件名（工作目录） | `table-<序>.jpg` | 无官方对应常量，本项目自定 | — | 本项目约定（落包时改为 `000N.jpg`） |
...
| R-13 | 落包时图片需从工作目录名（`drawing-1.jpg` 等）重命名为 `0001.jpg` 序列，`img/@file` 须同步改写 | 改写遗漏则 XML 与包内实际文件对不上 | 新增任务：`patent.js` 在打包阶段统一做「重命名 + 回写 `@file`」，并在 `xml-patent.test.js` 加断言 |
| R-14 | 五书在提交包内需按表格代码建目录（`100001`…`100005`）并生成 `List.xml` | 不生成 `List.xml` 则客户端无法识别案卷包 | 本期 `{name}.zip` 仍按可读文件名打包；生成合规案卷包（含 `List.xml`）列为后续增强，因其需要电子申请注册号等用户信息 |
