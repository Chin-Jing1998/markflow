...
- **转换**：拖放文件或文件夹，或在「网页链接」输入框中每行粘贴一个 `http`/`https` 地址批量提交；任务列表实时显示进度与告警，并发为 2。专利五书 XML 反向导入的三种输入在此一并受理：单个 `.xml`、案卷 `.zip` 与五书目录，缺省转为 docx（亦可改选 html／pdf／xml，`bundle` 不接受本类输入，故不出现在目标下拉里）。拖入或选中一个文件夹时，主进程先按目录签名判断它是不是五书目录：是则整个目录作为一项输入，任务列表标注「专利五书目录」；不是则照旧展开为其下受支持的文件，目录里的 `.xml` 与 `.zip` 不随之带入。队列中出现这类输入时，底栏多出「段号写进正文」开关（缺省关，等同命令行的 `--xml-import-paragraph-numbers`）；导入时读到了哪些书、多少处公式图等以「导入：」开头的告警随结果卡一并给出。
- **对比预览**：左栏为来源、右栏为产物；侧边格式面板按目标切换可调字段，排版类改动实时重渲染，解析类改动（图片格式、公式处理、PDF 后端、XML profile 及其子项、五书 XML 反向导入的段号）在同一会话内重新解析后重渲染；确认后导出并写入文件库。
