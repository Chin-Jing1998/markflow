...
v3.1.0 为专利五书的第二阶段：`patent` 方言的产物目录、文件命名、XML 字节形态与图片像素对齐官方「WORD 转 XML 编辑器」的真实产出；新增化学结构式归类、EMF 图元转 JPG、公式图与表格图保真、按 Word 分节页眉与官方模板标记识别五书、专利五书 XML 反向导入为可再编辑的 Word、Word for Mac 任务窗格加载项，以及桌面端的「XML 方言」选择与「XML 转 Word」入口。各项说明见下文对应章节。

v3.2.0 为修复版，不新增转换目标与选项，依赖不变：docx 与网页输入中的删除线、填空横线、强调、字面的「<」「&」与「~」、上下标与表题不再丢失或被改写，含孤立代理项字符引用的 docx 不再转换失败，Markdown 产物重新解析后结构保持不变；网页、Markdown、专利五书与 docx 链路上多处随输入长度超线性增长的耗时改为线性；桌面端同一轮转换中不同目录下的同名文件不再互相覆盖产物。逐项说明见 v3.2.0 的 Release 说明。
...
- **转换**：拖放文件或文件夹，或在「网页链接」输入框中每行粘贴一个 `http`/`https` 地址批量提交；任务列表实时显示进度与告警，并发为 2。队列里有目标为 `xml` 的待转任务时，底栏多出「XML 方言」下拉：「通用结构」按文档自身的层次输出单份 XML，「国知局五书」按官方案卷结构输出 `100001`–`100005` 五份 XML、同目录图片与案卷 zip（等同命令行的 `--xml-profile patent`）；初值取「设置 › 转换默认项」里的 XML 方言，未设置时为通用结构。同处的「DTD 校验」在国知局五书下按官方 DTD 校验，在通用结构下只做 well-formed 检查。专利五书 XML 反向导入的三种输入在此一并受理：单个 `.xml`、案卷 `.zip` 与五书目录，缺省转为 docx（亦可改选 html／pdf／xml，`bundle` 不接受本类输入，故不出现在目标下拉里）。拖入或选中一个文件夹时，主进程先按目录签名判断它是不是五书目录：是则整个目录作为一项输入，任务列表标注「专利五书目录」；不是则照旧展开为其下受支持的文件，目录里的 `.xml` 与 `.zip` 不随之带入。队列中出现这类输入时，底栏多出「段号写进正文」开关（缺省关，等同命令行的 `--xml-import-paragraph-numbers`）；导入时读到了哪些书、多少处公式图等以「导入：」开头的告警随结果卡一并给出。
- **对比预览**：左栏为来源、右栏为产物；侧边格式面板按目标切换可调字段，排版类改动实时重渲染，解析类改动（图片格式、公式处理、PDF 后端、XML profile 及其子项、五书 XML 反向导入的段号）在同一会话内重新解析后重渲染；确认后导出并写入文件库。格式面板只提交实际改动过的项与会话里本来就显式设过的项，没动过的字段显示的是实际会生效的缺省值——例如「JPG 分辨率（PPI）」在国知局五书方言下显示 300（官方只受理 72–300 DPI），在通用结构下显示 330；显式填入的数值一律以所填为准。
...
- **设置**：外观、默认输出目录、各输入类别的默认目标、转换默认项（含「XML 方言」：设为国知局五书后，转换页的方言下拉与对比预览的格式面板都以它为初值；留空即通用结构）、MinerU 令牌的保存与连通性测试、文件库模式、Word 加载项的启用与安装（见下文「Word for Mac 加载项」）、运行能力自检，以及「关于」——三端使用方法、联系与反馈、版本比对式的检测更新。
...
| `--font-east-asia <字体>` | 字体名 | `微软雅黑` | `docx` 中文字体；输入为专利五书 XML（`.xml`、案卷 `.zip`、五书目录）时缺省改取 `宋体` |
...
表格图的保真：docx 表格在解析层另存一份结构化的行列数据，出表格图时据此重建，合并单元格（`colspan`／`rowspan`）、单元格内的多个段落与粗体、斜体、下划线、上下标均予保留；首行不再强制加粗，源文档里标为「重复标题行」的行也不额外加粗（该标记在 Word 里只表示跨页重复），粗体只取自文字本身的格式。已知限制三项：对齐与列宽取不到（解析库 mammoth 不输出 `w:jc` 与 `w:tblGrid`）；单元格内的图片以替换文字占位并告警；嵌套表格按纯文本展开并告警。
...
桌面应用的转换页同样受理这三种输入：直接拖入或经「浏览文件」「选择文件夹」选中即可，五书目录会被整体识别为一项输入并标注「专利五书目录」，队列里出现这类输入时底栏的「段号写进正文」开关等同下文的 `--xml-import-paragraph-numbers`。
...
段落与段号在往返中保真：段首或段尾、显示宽度不小于约 53 mm（96 DPI 下 200 px）的段内图片，在 docx 解析层会被「大图拆段」规则拆成独立段落（为的是 Markdown 里 `<img>` 独占一行、图注能认回它所属的图片），转回五书 XML 时，权利要求书、说明书与说明书摘要按同组标记把同一个 Word 段落拆出的各块并回一个段落，段落数与段号均不变；说明书附图与摘要附图里则维持拆开——那两本书里「图片 + 同段图号」须分开才能认成附图与图号。
