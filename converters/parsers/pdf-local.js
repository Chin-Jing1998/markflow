 ...
 * PDF → IR（本地文本层后端）
 *
 * 契约：parse({ path }, ctx) → MarkFlowDocument，按绝对路径读取、不写盘；meta.pdfParser 恒为 'pdfjs'。
 * 由 parsers/pdf.js 分派：pdfBackend 为 local，或 auto 且未配置 MinerU 令牌时走本后端。
 *
...
async function buildDocument(pdf, { sourceName, ctx }) {
    ...
    return createDocument({
        ...
        meta: {
            ...
            sourceType: 'pdf',
            sourceName,
            // 分派器与 index.js 据此写出 backends.pdfParser 与 front matter 的 pdfParser 字段
            pdfParser: 'pdfjs',
        },
        assets: [],
        warnings: [],
